    ...
    fetch(`/edit_repair/${repairId}`)
        ...
        .then(data => {
            if (data.success) {
                const form = document.getElementById('edit-repair-form');
                if (!form) {
                    throw new Error('Edit repair form not found');
                }
                form.action = `/edit_repair/${repairId}`;

                // تعريف العناصر المطلوبة وقيمها
                const formElements = [
                    { id: 'edit_customer_id', value: data.repair.customer_id },
                    { id: 'device_id', value: data.repair.device_id },
                    { id: 'device-search', value: `${data.repair.device_name} - ${data.repair.device_category}` },
                    { id: 'edit_issue_id', value: data.repair.issue.id },
                    { id: 'edit_status', value: data.repair.status },
                    { id: 'edit_repair_price', value: data.repair.repair_price },
                    { id: 'edit_notes', value: data.repair.notes || '' }
                ];
                ...
                formElements.forEach(element => {
                    ...
                    if (!inputElement) {
                        throw new Error(`Form element ${element.id} not found`);
                    }
                    inputElement.value = element.value;
                });

                if (!window.editRepairModal) {
                    throw new Error('Edit repair modal not initialized');
                }
                window.editRepairModal.show();
            } else {
                throw new Error(data.message || 'Failed to load repair data');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showToast(error.message || 'Error loading repair request', false);
        });
}
    ...
    fetch(`/get_repair_details/${repairId}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('review-issue').textContent = data.repair.issue.title;
...
function updateRepairStatus(repairId, newStatus) {
    ...
    fetch(`/update_repair_status/${repairId}`, {
        method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .then(data => {
        if (data && data.success) {
            ...
            const statusBtn = document.getElementById(`statusDropdown-${repairId}`);
            if (statusBtn) {
                // Update button classes based on new status
                statusBtn.className = `text-xs font-medium px-2.5 py-1 rounded-full inline-flex items-center gap-1 ${ 
                    newStatus === 'In Progress' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' :
                    newStatus === 'Pending' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' :
                    'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300'
                }`;
                // Update status text
                statusBtn.firstChild.textContent = newStatus;
            }
            showToast(data.message || 'تم تحديث حالة الإصلاح بنجاح');
        }
    })
    .catch(error => {
        console.error('Error updating repair status:', error);
        showToast('حدث خطأ أثناء تحديث حالة الإصلاح', false);
    });
}
